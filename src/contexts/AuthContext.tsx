...
export function signOut(){
    try{
        ...
        destroyCookie(undefined, '@cardapioplus.token')
...
export function AuthProvider({children}: AuthProviderProps){
    ...
    async function signIn({email, password}: SignProps){
        try{
            ...
            const { idUser, name, token } = response.data

            setCookie(undefined, '@cardapioplus.token', token, {
    ...
    useEffect(()=>{
        ...
        const {'@cardapioplus.token': token} = parseCookies();

        // a rota get /user recebe um token de autenticação e verifica a veracidade dele. caso seja um token valido, retorna os dados do user que estão inseridos nele e atualiza os valores do useState user. É feita essa atualização sempre para que possamos ter os dados do user em todas as paginas
        // nao precisa enviar o token para essa rota, porque se o user ja tiver feito o login, a api busca pelo token no cookie e adiciona no header no atributo 'authorization' (Authorization: `Bearer ${cookies['@cardapioplus.token']}`). configurado em ./api.ts
        ...
        if(token){
            api.get('/user').then(response => {
                const { idUser, name, email } = response.data;
            ...
            .catch((err)=>{
                ...
                console.log("ERRO DE AUTENTICACAO: ", err)
                signOut()
            })
